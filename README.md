...
2. Copy your other SmartApp nodeJS files (e.g. mysecondapp.js, etc) to your smartapp project directory, and copy any language files (e.g. mysecondapp_en.json) to the *locales* subdirectory.
  - *IMPORTANT NOTE:  There is an issue with the SmartThings SDK at the moment with handling multiple language files, so you may need to copy your json *into* the locales/mqttout_en.json file or visa versa - whatever module gets loaded last will be the one the SDK uses.*
